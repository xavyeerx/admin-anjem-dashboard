...
export default function PaymentsPage() {
  ...
  const { data, loading, error, refetch } = usePaymentTracking();
  ...
  const handleMarkPaid = async (item: TrackingItem) => {
    ...
    await runAction({
      ...
      action: async () => {
        ...
        const res = await markTrackingPaid(item.membership_id);
      ...
      <div className="animate-fade-in delay-2" style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "16px 20px", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          {[
            ...
            { label: "→" },
            { label: "Status: Belum Bayar", color: "var(--amber)" },
            { label: "→" },
            { label: "Jatuh Tempo (Mulai + 3 Hari)", color: "var(--amber)" },
            { label: "→" },
          ...
          <table style={{ width: "100%" }}>
            ...
            <tbody>
              {processedItems.map((item, i) => {
                ...
                const avatarColor = isLewat ? "var(--red)" : "var(--amber)";
                const avatarBg   = isLewat ? "var(--red-glow)" : "var(--amber-glow)";
                const avatarBorder = isLewat ? "rgba(220,38,38,0.3)" : "rgba(217,119,6,0.2)";
