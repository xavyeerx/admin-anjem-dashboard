...
// GET /api/shareholders — fetch config + computed profit distribution
export async function GET() {
  return withErrorHandler(async () => {
    ...
    // Total expenses
    const { data: expenses } = await db.from("expenses").select("nominal");
    const totalPengeluaran = (expenses ?? []).reduce((s: number, e: { nominal: number }) => s + e.nominal, 0);

    const labaBersih = totalPemasukan - totalPengeluaran;

    // Transfers to Bintang
    const { data: transfers } = await db.from("transfers").select("nominal");
...
// PUT /api/shareholders — update shareholder percentages
  ...
  return withErrorHandler(async () => {
    ...
    const results = await Promise.all(
      updates.map(({ id, persentase }) =>
        db.from("shareholders").update({ persentase }).eq("id", id).select().single(),
      ),
    );
