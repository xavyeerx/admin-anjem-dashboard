...
/**
 * GET /api/[cabang]/dashboard/stats
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ cabang: string }> }) {
  return withErrorHandler(async () => {
    ...
    const [driversRes, membershipsRes, expensesRes, transfersRes] = await Promise.all([
      db.from("drivers").select("id, nama, status_operasional, jenis_driver").eq("cabang_id", cabang),
      db.from("memberships").select("id, status_pembayaran, jenis_driver, nominal, driver_id, tanggal_mulai, deadline_pembayaran, tanggal_selesai_final, drivers(id, nama, no_whatsapp, jenis_driver)").eq("cabang_id", cabang),
      db.from("expenses").select("id, nominal, kategori, tanggal").eq("cabang_id", cabang),
      db.from("transfers").select("id, nominal").eq("cabang_id", cabang),
    ]);
    ...
    // ---- Shareholder summary ----
    const { data: shareholders } = await db.from("shareholders").select("*").eq("cabang_id", cabang).order("persentase", { ascending: false });
