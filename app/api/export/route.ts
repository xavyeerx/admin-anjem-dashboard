...
async function fetchDrivers(db: ReturnType<typeof createServiceClient>) {
...
async function fetchMembership(db: ReturnType<typeof createServiceClient>) {
  const { data, error } = await db
    .from("memberships")
    .select("*, drivers(nama)")
...
async function fetchKeuangan(db: ReturnType<typeof createServiceClient>) {
  const [{ data: memberships, error: e1 }, { data: expenses, error: e2 }, { data: transfers, error: e3 }] =
    await Promise.all([
      db.from("memberships").select("tanggal_mulai, nominal, status_pembayaran, tanggal_pembayaran").order("tanggal_mulai", { ascending: false }),
      db.from("expenses").select("*").order("tanggal", { ascending: false }),
      db.from("transfers").select("*").order("tanggal", { ascending: false }),
    ]);
...
async function fetchShareholder(db: ReturnType<typeof createServiceClient>) {
  const [{ data: shareholders, error: e1 }, { data: transfers, error: e2 }] = await Promise.all([
    db.from("shareholders").select("*").order("persentase", { ascending: false }),
    db.from("transfers").select("*").order("tanggal", { ascending: false }),
  ]);
  if (e1) throw new Error(e1.message);
  if (e2) throw new Error(e2.message);
...
export async function GET(req: NextRequest) {
  try {
    ...
    switch (type) {
      case "drivers":
        rows = await fetchDrivers(db);
        sheetName = "Data Driver";
        filename = "data-driver";
        break;
      case "membership":
        rows = await fetchMembership(db);
        sheetName = "Membership";
        filename = "data-membership";
        break;
      case "keuangan":
        rows = await fetchKeuangan(db);
        sheetName = "Keuangan";
        filename = "laporan-keuangan";
        break;
      case "shareholder":
        rows = await fetchShareholder(db);
