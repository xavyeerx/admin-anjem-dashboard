...
export default function FinancePage() {
  ...
  const { data: memRows, loading: memLoading, refetch: refetchMem } = useMemberships();
  const { data: expRows, loading: expLoading, refetch: refetchExp } = useExpenses();
        ...
        <div style={{ display: "flex", gap: 0, marginBottom: 0, borderBottom: "1px solid var(--border)" }}>
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
              ...
              border: "none", borderBottom: `2px solid ${activeTab === tab.key ? "var(--amber)" : "transparent"}`,
