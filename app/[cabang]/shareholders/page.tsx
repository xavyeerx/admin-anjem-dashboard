...
export default function ShareholdersPage() {
  ...
  const { data: shData, loading: shLoading, refetch: refetchSh } = useShareholders(cabang);
  const { data: transfers, loading: trLoading, refetch: refetchTr } = useTransfers(cabang);
  ...
  const handleSaveConfig = async () => {
    ...
    await runAction({
      ...
      action: async () => {
        const result = await updateShareholders(cabang, updates);
  ...
  const handleSaveTransfer = async () => {
    ...
    await runAction({
      ...
      action: async () => {
        const result = await createTransfer(cabang, {
  ...
  const handleDeleteTransfer = async (id: string, keterangan: string) => {
    ...
    await runAction({
      ...
      action: async () => {
        const result = await deleteTransfer(cabang, id);
        if (result.error) throw new Error(result.error);
        refetchTr();
        refetchSh();
      },
    ...
    <div style={{ padding: "var(--page-py) var(--page-px)", maxWidth: 1000, position: "relative" }}>
        ...
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: "20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              { label: "Sisa",           value: sisaBintang,   color: sisaBintang > 0 ? "var(--brand)" : "var(--green)" },
            ].map((row) => (
      ...
      <Modal open={showConfigModal} onClose={() => setShowConfigModal(false)} title="Konfigurasi Persentase Saham">
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "var(--bg-elevated)", borderRadius: 8, padding: "12px 14px", border: "1px solid var(--border)", fontSize: 12, color: "var(--brand)" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            ...
            <button onClick={handleSaveConfig} disabled={totalConfigPct !== 100} style={{
              padding: "9px 16px",
              background: totalConfigPct === 100 ? "var(--brand)" : "var(--bg-elevated)",
              color: totalConfigPct === 100 ? "#fff" : "var(--text-muted)",
