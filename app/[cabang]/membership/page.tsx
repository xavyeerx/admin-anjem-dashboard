...
export default function MembershipPage() {
  ...
  const { data: driverRows } = useDrivers(cabang);
  const drivers = (driverRows ?? []).map(fromDriverRow);

  const { data: rows, loading, error, refetch } = useMemberships(cabang, {
  ...
  const handleSave = async () => {
    ...
    try {
      await runAction({
        ...
        action: async () => {
          const result = isEdit
            ? await updateMembership(cabang, editingId!, payload)
            : await createMembership(cabang, payload);
  ...
  const handleMarkPaid = async (id: string, driverName: string) => {
    await runAction({
      ...
      action: async () => {
        const result = await markMembershipPaid(cabang, id);
        if (result.error) throw new Error(result.error);
        refetch();
      },
    });
  ...
  const handleDelete = async (id: string, driverName: string) => {
    ...
    await runAction({
      ...
      action: async () => {
        const result = await deleteMembership(cabang, id);
        if (result.error) throw new Error(result.error);
        refetch();
      },
    });
          ...
          {["all", "Lunas", "Belum Bayar", "Lewat Jatuh Tempo"].map((f) => (
            <button
              ...
              style={{
                ...
                borderColor: filterStatus === f ? "var(--brand)" : "var(--border)",
                background: filterStatus === f ? "var(--brand-glow)" : "transparent",
                color: filterStatus === f ? "var(--brand)" : "var(--text-secondary)",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <FormField label="Selesai Final">
              <div style={{ ...readonlyDateStyle, borderColor: "var(--brand)", color: "var(--brand)" }}>
