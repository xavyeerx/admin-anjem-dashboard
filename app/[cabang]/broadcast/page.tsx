...
export default function BroadcastPage() {
  ...
  const handleGenerate = async () => {
    ...
    setBroadcast(null);
    try {
      const res = await fetch(`/api/${cabang}/broadcast`);
      if (!res.ok) {
  ...
  return (
    <div style={{ padding: "var(--page-py) var(--page-px)", maxWidth: 900 }}>
      ...
      <div className="animate-fade-in delay-1" style={{
        ...
        <Zap size={18} color="var(--brand)" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          <div style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)", marginBottom: 4 }}>
            Cara Pakai
          </div>
          <div style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.7 }}>
            Klik <strong style={{ color: "var(--brand)" }}>Generate Broadcast</strong> untuk membuat teks broadcast secara otomatis berdasarkan data driver terkini.
