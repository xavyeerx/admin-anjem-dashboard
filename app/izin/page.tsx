...
export default function IzinPage() {
  ...
  const { data: driverRows, loading: driverLoading } = useDrivers();
  const drivers = driverRows ?? [];

  const { data: izinData, loading: izinLoading, refetch } = useIzin();
  ...
  const handleDelete = async (id: string, driverName: string) => {
    ...
    await runAction({
      ...
      action: async () => {
        const result = await deleteIzin(id);
        if (result.error) throw new Error(result.error);
        refetch();
      },
    });
  ...
  return (
    <div style={{ padding: "28px 32px", position: "relative" }}>
      ...
      <div className="animate-fade-in delay-1" style={{
        ...
        <CalendarOff size={18} color="var(--amber)" style={{ flexShrink: 0, marginTop: 2 }} />
        <div>
          ...
          <div style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.7 }}>
            ...
            Contoh: Masa aktif 01-14 Jun + 3 hari izin → Selesai Final 17 Jun. Riwayat izin tetap tersimpan.
          </div>
        </div>
      </div>
                ...
                <tr key={izin.id}>
                  ...
                  <td>
                    <span style={{
                      ...
                      background: "var(--blue-glow)", border: "1px solid rgba(59,130,246,0.3)",
                      color: "var(--blue)", padding: "4px 12px", borderRadius: 99,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <FormField label="Jumlah Hari Izin">
            <input
              ...
              onChange={(e) => setForm({ ...form, jumlahHari: e.target.value })}
              style={inputStyle}
            />
          </FormField>
          <FormField label="Alasan Izin">
            <textarea
              placeholder="Alasan pengambilan izin..."
              value={form.alasan}
          ...
          {/* Preview dihilangkan karena sulit dihitung di client-side tanpa data membership real-time lengkap per form,
              namun logic backend sudah 100% aman mengupdate tanggal selesai final */}
